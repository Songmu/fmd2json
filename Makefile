...
BUILD_LDFLAGS = "-s -w -X github.com/Songmu/fmd2json.revision=$(CURRENT_REVISION)"
u := $(if $(update),-u)
...
.PHONY: build
build:
	go build -ldflags=$(BUILD_LDFLAGS) ./cmd/fmd2json

.PHONY: install
install:
	go install -ldflags=$(BUILD_LDFLAGS) ./cmd/fmd2json
